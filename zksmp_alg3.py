...
def hash_sigma(sigma):
    sigma_bytes = str(sigma).encode('utf-8')
    hashed_sigma = hashlib.sha256(sigma_bytes).hexdigest()
    return int(hashed_sigma, 16)
...
def commit(g, h, sigma, r):
    ...
    return (g ** hashed_sigma) * (h ** r)

###########################
# Prover & Verifier (表3.3)
###########################
class Prover:
    ...
    def step5_send_z(self, c):
        """
        步骤5:
          z_sigma= s - h(sigma)*c, z_v= t - v*c, z_r= m - r*c
        """
...
class Verifier:
    def __init__(self, pairing, g, h, Phi, C):
        """
        pairing, g, h, Phi, C= g^h(sigma) h^r
        """
        self.pairing = pairing
        self.g = g
        self.h = h
    ...
    def pick_x_and_send_yAi(self):
        ...
        A_dict = {}
        for i in self.Phi:
            Ai = self.g ** (~Element(self.pairing, Zr, str(self.x + hash_sigma(i))))
            A_dict[i] = Ai
...
def time_protocol_generation(Phi, sigma):
    ...
    return {
        ...
        "pickV_time": pickV_time,     # 相当于"Sign Time"
